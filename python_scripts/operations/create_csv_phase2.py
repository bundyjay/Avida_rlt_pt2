...
from data_dict_phase2 import create_data_dict as create_data_dict
from sort_csv_phase2 import sort_csv as sort_csv
analysis_directory = str(project_folder) + '/output_analysis'

def  create_csv(
    data_dict=create_data_dict(
        log_name='data_dict_phase2_log.txt'
        ), 
    csv_name='phase2_raw.csv'
    ):
...
def create_and_sort_csv():
    ...
    yield print(
            sort_csv(
                csv_name='phase2_raw.csv', 
                sorted_name='phase2_dataframe.csv'
                ))
