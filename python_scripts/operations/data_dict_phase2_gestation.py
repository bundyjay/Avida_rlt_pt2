...
2. Find the ancestral gestation and genome length.
...
def create_data_dict(log_name='data_dict_phase2_log.txt'):
    ...
    trajectory_updates = [x for x in range(0,101000,1000)] 
    for each_input_folder in input_subfolders: 
                    ...
                    for line in lines:
                        ...
                        if update_0_organism:
                            for line in lines:
                                ...
                                ancestor_gestation_line = (
                                    search(
                                        r'Gestation Time\.\.: '
                                        r'([0-9,.e+]+)\n', line
                                        ))
                                if ancestor_gestation_line:
                                    ancestor_gestation = Decimal(
                                        ancestor_gestation_line.group(1)
                                        ) 
                    ...
                    for each_alternate_file in listdir(
                        ...
                        with open(alternate_file_location, 'r+')  \
                            ...
                            for alt_line in alternate_lines:
                                ...
                                if update_0_organism:
                                    for alt_line in alternate_lines:
                                        ancestor_gestation_line = search(
                                                r'Gestation Time'
                                                r'\.\.: '
                                                r'([0-9,.e+]+)\n', 
                                            alt_line
        ...
        for each_update in trajectory_updates:
            ...
            data_dict[gestation_key].update({
                'series': gestation_series, 
                ...
                'time': update, 
                'trait': 'gestation', 
                'ancestor_value': ancestor_gestation, 
                'ancestor_value_log10': ancestor_gestation.log10()
                })
            length_key = (
                'Environment%s_'
                'Depth%s_'
                        ...
                        if update == 0:
                            data_dict[gestation_key].update({
                                'evolved_population_average_value': (
                                    data_dict[gestation_key]['ancestor_value']
                                    ), 
                                'evolved_population_average_value_log10': ((
                                    data_dict[gestation_key]
...
if __name__ == '__main__':
    create_data_dict(log_name='data_dict_phase2_gestation_log.txt')
